...
$(function() {
    $("#login-modal-form").submit(function(e) {
        e.preventDefault();
        $("#modal-login-submit").attr('disabled', 'disabled');

        $.post( "/api/v1/try-to-login", $("#login-modal-form").serialize())
            .done(function() {
                $("#modal-login-form-errors").hide();
                $("#modal-login-form-errors").html('');
                $.post("/login_check", $("#login-modal-form").serialize())
                    .done(function() {
                        window.location.href = "/";
                    })
            })
            .fail(function(error){
                $("#modal-login-form-errors").html(error.responseJSON);
                $("#modal-login-form-errors").show();
                $("#modal-login-submit").removeAttr('disabled');
                return false;
            })
    });
});